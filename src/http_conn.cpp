...
int setnonblocking(int fd)
{
    ...
    flag |= O_NONBLOCK;
    return fcntl(fd, F_SETFL, flag); // -1 error;
}
...
int addfd(int epollfd, int fd, bool one_shot)
{
    epoll_event event;
    event.data.fd = fd;
    // event.events = EPOLLIN | EPOLLRDHUP; // EPOLLRDHUP端断开连接
    event.events = EPOLLIN | EPOLLRDHUP; // EPOLLRDHUP端断开连接

    if (one_shot)
    {
        event.events |= EPOLLONESHOT; //只检测一次文件描述符的变化
    }
...
/*重置socket上的EPOLLONESHOT，以确保下一次可读时，可以触发,这是one_shot原理决定的
注册了 EPOLLONESHOT 事件的 socket 一旦被某个线程处理完毕， 该线程就应该立即重置这个
...
// 解析HTTP请求的一个头部信息
http_conn::HTTP_CODE http_conn::parse_request_hander(char *text)
{
    // 遇到空行，表示头部字段解析完毕
    if (text[0] == '\0')
    {
